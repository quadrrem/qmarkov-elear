...
(defn word-chain [word-transitions]
  (reduce (fn [r t]
    (merge-with clojure.set/union r
      (let [[a b c] t] {[a b] (if c #{c} #{})}))) {} word-transitions))
...
(defn chain->text [chain] (apply str (interpose " " chain)))
...
(defn tweet-text []
  (let [text (generate-text (first (shuffle prefix-list)) functional-leary)
        result-text (end-at-last-punctuation text)
        result-word-count (count (clojure.string/split result-text #" "))]
        (if (>= result-word-count 6) result-text (recur))))
...
(defn status-update []
  (let [tweet (tweet-text)]
    ...
    (when (not-empty tweet)
      (try
        (twitter/statuses-update :oauth-creds my-creds :params {:status tweet})
